# -*- coding: utf-8 -*-
"""
plot_walk_result_hybrid.py — 双中心壳修正 5D 随机行走结果可视化
====================================================================
读 results/随机行走/walk_5d_hybrid.npz 与 path_5d_hybrid.npz：
  - 图1: E(elong) 剖面（行走散点 + 包络 + 极小能量路径）
  - 图2: neck / η / ε1 / ε2 演化（重点看断裂区 η 是否偏到非对称、ε1>ε2）
"""
...
def main():
    ...
    wp, we = load("walk_5d_hybrid.npz")
    mp, me = load("path_5d_hybrid.npz")
    ...
    ax.set_title("5D 随机行走 vs 极小能量路径（断裂区双中心壳修正，U-236）", fontsize=12)
    ax.legend(loc="upper left", fontsize=9)
    fig.tight_layout()
    out1 = os.path.join(FIG, "walk_vs_path_energy_hybrid.png")
    ...
    fig.suptitle("随机行走 5 参数演化（断裂区 η→非对称、ε1>ε2）", fontsize=12)
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    out2 = os.path.join(FIG, "walk_5d_evolution_hybrid.png")
